...
class SyncApiPropertyImportController extends Controller
{
 ...
 //$removedProperty    = ['Sold'];
 
      
      for($r=0; $r < count($responseBody); $r++){
            // if($responseBody[$r]['propertyStatus'] != "Auction" || $responseBody[$r]['propertyStatus'] != "Rental Daily" || $responseBody[$r]['propertyStatus'] != "Pending Sale" || $responseBody[$r]['propertyStatus'] != "Inactive" ){
...
$commercialArray = ['Commercial','Game Farm Lodge','Guest House','Industrial','Industrial Land','Office','Restaurant','Retail','Shop','Vacant Land Commercial'];
                    ...
                    if (in_array($responseBody[$r]['propertyType'], $residentialArray))
                      {
                       $insertEntegralData->property_classification = 'residential';
                      }
                    else if(in_array($responseBody[$r]['propertyType'], $commercialArray))
                      {
            ...
            $insertEntegralData->country = $responseBody[$r]['country'];
            
            $insertEntegralData->province = $responseBody[$r]['province'];
            
            $insertEntegralData->town = $responseBody[$r]['town'];
            
            $insertEntegralData->suburb = $responseBody[$r]['suburb'];
            ...
            $insertEntegralData->floor_size = NULL ;
            $insertEntegralData->floor_size_unit = NULL;
            ...
            $insertEntegralData->headline = ucfirst(strtolower($responseBody[$r]['title']));
