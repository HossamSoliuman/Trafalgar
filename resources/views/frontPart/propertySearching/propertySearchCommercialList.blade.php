@php $checkPageType = Request::segment(2) ;  @endphp

@if($checkPageType != "multiple-areas")
@section('meta_keywords','COMMERCIAL CITY FOR SALE')
@php if(isset($_GET['city'])){ $cityTitle = $_GET['city']; }else{ $cityTitle = ""; }  @endphp
@php if(isset($_GET['city'])){ $cityDesc = $_GET['city']; }else{ $cityDesc = ""; }  @endphp

@section('title','Commercial Property for sale in '.$cityTitle.' : '.$cityTitle.' Property')
@section('meta_description','Commercial Property for sale in '.$cityDesc.'. The largest selection of office, retail & industrial property To Rent in '.$cityDesc.' by estate agents.')

@else
@section('meta_keywords','COMMERCIAL MULTIPLE SUBURBS FOR SALE')
@section('title','Commercial property for sale in Multiple Suburbs')
@section('meta_description','Commercial property for sale in Multiple Suburbs.')
@endif
...
@section('content')
    ...
    <section class="guldenland_section">
        <div class="container">
            <div class="heading mb-4">
               ...
               @endphp
               <h2>Property for sale in <?php if( (isset($_GET['city']) && $_GET['city'] !="")){ echo $text; } ?></h2>
            </div>
                            ...
                            <div style="cursor: pointer;" onclick="window.location = '{{ $customUrl }}';" class="guldenland_each_product">
                                <div class="product_img">
                                   <a title="{{ $propertyData->complex_name." ".$propertyData->unit_number }}" href="{{ $customUrl }}">
                                        ...
                                        <img  class="img-fluid" src="{{ $propertyData->news_featured_image }}" alt="{{ ($propertyData->headline != "")?$propertyData->news_featured_image:$propertyData->news_featured_image }}" />
                                         @else
                                   <img title="{{ $propertyData->complex_name." ".$propertyData->unit_number }}" class="img-fluid" src="{{ asset('storage/propertyDummyImage/dummy_property_image.jpg') }}"  alt="{{ ($propertyData->headline != "")?$propertyData->headline:'dummy_property_image.jpg' }}" />
                                   @endif
                                ...
                                <div class="product_content">
                                    ...
                                    <small title=' @if($propertyData->bedrooms != 0)
                                    {{ $propertyData->bedrooms." Bedroom" }}
                                    @elseif(strtolower($propertyData->property_type) == 'apartment' )
                                    Bachelor
                                    @endif {{ ucwords($propertyData->property_type." ".$propertyData->mandate_saletype." "."in". " ".$propertyData->suburb) }}' > @if($propertyData->bedrooms != 0)
                                    {{ $propertyData->bedrooms." Bedroom" }}
                                    @elseif(strtolower($propertyData->property_type) == 'apartment' )
                                    Bachelor
                                    @endif {{ ucwords($propertyData->property_type." ".$propertyData->mandate_saletype." "."in". " ".$propertyData->suburb) }} </small>
    ...
    <section class="footer_top_section footer_top_section_bgwhite">
      ...
      $customUrl_distinct = route('commercial-property-for-sale',['url_city'=>$suburb_distinct,'url_town'=>$town_distinct,'url_province'=>$province_distinct,'city'=>$getDistinctSuburbs->suburb]);
...
@push('property-search-commercial-list-js')
@include('frontPart.js.propertySearchCommercialListJs')
@endpush
