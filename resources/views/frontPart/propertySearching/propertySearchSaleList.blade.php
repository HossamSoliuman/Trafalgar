...
@if ($checkPageType != 'multiple-areas')
    @section('meta_keywords', 'RESIDENTIAL CITY FOR SALE')

    @php
        if (isset($_GET['city'])) {
            $cityTitle = $_GET['city'];
        } else {
            $cityTitle = str_replace('-', ' ', Request::segment(2));
        }

        if (isset($_GET['city'])) {
            $cityDesc = $_GET['city'];
        } else {
            $cityDesc = str_replace('-', ' ', Request::segment(2));
        }

        if (isset($url_town)) {
            $citytown = $url_town;
        } else {
            $citytown = str_replace('-', ' ', Request::segment(2));
        }

        $shortenCity = Str::limit($cityTitle, 15, '');

        $shortenName = $shortenCity . ' ' . $citytown;

        $metaTitle = 'Property and houses for sale in ' . $shortenName;

        $shortenCityDesc = Str::limit($cityDesc, 20, '');
        $shortenDescName = $shortenCityDesc . '-' . $citytown;

        $metaDescription =
            'Property for sale in ' .
            $shortenDescName .
            ', A large selection of flats, farms, townhouses & houses for sale in ' .
            $shortenCityDesc;
...
@else
    @section('meta_keywords', 'RESIDENTIAL MULTIPLE SUBURBS FOR SALE')
    @section('title', 'Houses for sale in Multiple Suburbs')
    @section('meta_description', 'Flats, apartments, townhouses and houses for sale in Multiple Suburbs.')
@endif
                    ...
                    <div class="property-section-content">
                        <div class="content">
                            ...
                            <form action="{{ route('property') }}" method="get" id="searchForm" class="resetFormData">
                                        ...
                                        <div class="select_buttons">
                                            <div class="single_select_buttons select_icon">
                                                <select id="stype" class="form-select"
                                                    ...
                                                    <option <?php if ($getUriSegmentForSaleOrRent == 'for-rent') {
                                                        echo 'selected';
                                                    } ?> value="for rent">To Rent</option>
                                                    <option <?php if ($getUriSegmentForSaleOrRent == 'for-sale') {
                                                        echo 'selected';
                                                    } ?> value="for sale">For Sale</option>
                                                </select>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <input type="hidden" name="stype" id="setStype" value="for rent">
                                ...
                                @foreach ($propertyDatas as $propertyData)
                                    ...
                                    <div style="cursor: pointer;" onclick="window.location = '{{ $customUrl }}';"
                                        ...
                                        <div class="product_content">
                                            ...
                                            <small
                                                title=' @if ($propertyData->bedrooms != 0) {{ $propertyData->bedrooms . ' Bedroom' }}
                            ...
                            @if (isset($_GET['city']))
                                ...
                                <h4>
                                    @if (!empty($cityArray))
                                        ...
                                        ?>
                                    @endif Property For Sale
                                </h4>
                                <ul>
                                    @foreach ($sideBarCodeForSearch as $sideBarCodeForSearchs)
                                        @if ($cityArray[0] != '')
                                            <li><a
                                                    href="{{ route('for-sale', ['url_city' => str_replace(' ', '-', $sideBarCodeForSearchs->suburb), 'url_town' => str_replace(' ', '-', $sideBarCodeForSearchs->town), 'url_province' => str_replace(' ', '-', $sideBarCodeForSearchs->province), 'city' => $cityArray[0], 'ptype' => $sideBarCodeForSearchs->property_type]) }}">
                                                    {{ ucfirst($sideBarCodeForSearchs->property_type) }} for sale in
                                                    {{ $sideBarCodeForSearchs->suburb }} </a></li>
                                        @endif
                                    @endforeach
                                </ul>
                            @endif
                                    ...
                                    @if (!empty($getDistinctSuburb))
                                        @foreach ($getDistinctSuburb as $getDistinctSuburbs)
                                            @php
                                                ...
                                                $customUrl_distinct = route('for-sale', [
...
@push('property-search-sale-list-js')
    @include('frontPart.js.propertySearchSaleListJs')
@endpush
