...
class CommercialPropertyController extends Controller
{
   public function index(Request $request){
        ...
        $sr .= " AND  mandate_saletype = 'for sale'";
       ...
       $dataVal = EntegralApiData::WhereRaw($sr)->toSql();
        ...
        if (isset($request->sortType) && $request->sortType != "")
        {
            ...
            else if ($request->sortType == "alphabat")
            {
                ...
                //$propertyDatas = EntegralApiData::WhereRaw($sr)->orderBy('headline')->paginate(8);
        ...
        //         ->orderBy('suburb', 'asc')
        //         ->get();     
        $getDistinctSuburb = EntegralApiData::where('mandate_saletype','for sale')->where('property_classification','commercial')->inRandomOrder()->groupBy('suburb')->take(30)->get();
         // footer suburb array  
        ...
        $cookie = Cookie::queue(Cookie::make('searchKeyword', $request->city, 360));
         ...
         $cookie = Cookie::queue(Cookie::make('searchUrls', route('commercial-property-for-sale',['url_city'=>$url_city,'url_town'=>$url_town,'url_province'=>$url_province]), 360));
           // add search keyword with cookies
...
$sideBarCodeForSearch = $this->sideBarCodeForSearch($request);
        return view('frontPart.propertySearching.propertySearchCommercialList', compact('setting', 'propertyDatas', 'propertyTypeList','getDistinctSuburb','getSearchTaggerData','sideBarCodeForSearch','searchResultCookie'),['url_city'=>$suburb,'url_town'=>$town,'url_province'=>$province]);
    ...
    public function sideBarCodeForSearch($request){
        ...
        $srNew .= " AND  mandate_saletype = 'for sale'";
