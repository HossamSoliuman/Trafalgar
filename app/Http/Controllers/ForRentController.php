...
class ForRentController extends Controller
{
    public function toRent(Request $request)
    {
        ...
        // echo $name = $request->input('multiple-areas');
        //  echo $request()->get('param') ;
        ...
        if (isset($request->sortType) && $request->sortType != "") {
            ...
            if ($request->sortType == "mostrecent") {
                ...
                //$propertyDatas = EntegralApiData::WhereRaw($sr)->orderBy('CASE WHEN complex_name IS NULL THEN street_name ELSE complex_name END')
        ...
        // $getDistinctSuburb = EntegralApiData::distinct('suburb')->select('suburb')->WhereRaw($sr)->orderBy('suburb', 'asc')->get();  
        $getDistinctSuburb = EntegralApiData::where('mandate_saletype', 'for rent')->where('property_classification', 'residential')->inRandomOrder()->groupBy('suburb')->take(30)->get();
        // footer suburb array  
        ...
        $cookie = Cookie::queue(Cookie::make('searchUrls', route('to-rent', ['url_city' => $url_city, 'url_town' => $url_town, 'url_province' => $url_province]), 360));

        // foreach ($propertyDatas as $item) {
        //     $existIds[] = $item['property_id'];
        // }

        // $existIds[] = count($existIds);
        // return $existIds;
        ...
        $propertyTypeList = EntegralApiData::select('property_type')->where('property_classification', 'residential')->groupBy('property_type')
            ->get();

        // return $propertyDatas;
        $searchResultCookie = $this->searchResultCookie();
        $sideBarCodeForSearch =    $this->sideBarCodeForSearch($request);
        return view('frontPart.propertySearching.propertySearchRentList', compact('setting', 'propertyDatas', 'searchReferences', 'propertyTypeList', 'getDistinctSuburb', 'getSearchTaggerData', 'sideBarCodeForSearch', 'searchResultCookie'), ['url_city' => $suburb, 'url_town' => $town, 'url_province' => $province]);
    }
    ...
    public function getprovience(Request $request)
    {
        ...
        $cityrentdata = trim($request->cityrentdata, '@@');

        $multiparam = str_replace('@@', '&', $cityrentdata);
        $multiparam1 = str_replace(' ', '-', $multiparam);

        $explode_city = explode('@@', $cityrentdata);
        ...
        if (count($explode_city) == 1) {
            ...
            if (isset($agn_exp[0]) && isset($agn_exp[1])) {
                ...
                $check_for_town = EntegralApiData::where('mandate_saletype', 'for rent')->where('town', $agn_exp[0])->get();

                if (!$check_for_town->isEmpty()) {

                    //DB::enableQueryLog();
                    $propertyDatas =   EntegralApiData::where('mandate_saletype', 'for rent')->where('town', trim($agn_exp[0]))->where('province', trim($agn_exp[1]))->first();

                    //$quries = DB::getQueryLog();
                    if ($propertyDatas->town == '') {
                        $propertyDatas->town = '';
                    } else {
                        $propertyDatas->town = $propertyDatas->town;
                    }

                    if ($propertyDatas->province == '') {
                        $propertyDatas->province = '';
                    } else {
                        $propertyDatas->province = $propertyDatas->province;
                    }
                    ...
                    if (!$propertyDatas) {
                        ...
                        $customUrl = route('to-rent', ['url_town' => $town, 'url_province' => $province, 'url_city' => $suburb]);
                    }
                } else {
                    //DB::enableQueryLog();
                    $propertyDatas = EntegralApiData::where('mandate_saletype', 'for rent')->where('suburb', trim($agn_exp[0]))->where('town', trim($agn_exp[1]))->first();
                    //  $quries = DB::getQueryLog();
                    ...
                    if (!$propertyDatas) {
                        ...
                        $province = str_replace(" ", "-", $propertyDatas->province);
                        $status =  "single_city_name";
                        $customUrl = route('to-rent', ['url_city' => $suburb, 'url_town' => $town, 'url_province' => $province]);
                    }
                }

                //echo count($propertyDatas);
                ...
                $propertyDatas =   EntegralApiData::where('mandate_saletype', 'for rent')->where('province', trim($explode_city[0]))->first();
                ...
                if (!$propertyDatas) {
                    ...
                    $province = str_replace(" ", "-", $propertyDatas->province);
                    $status =  "single_city_name";
                    $customUrl = route('to-rent', ['url_city' => 1, 'url_town' => 2, 'url_province' => $province]);
                }
            }
            ...
            if ($status == "property_not_avaliable") {
                ...
                $propertyDatas =   EntegralApiData::Where('property_id', $cityrentdata)->first();

                $status =  "contain_property_id";
                // /property-to-rent/{p_suburb}/{p_town}/{p_province}/{p_id}/{p_ref}
            ...
            $customUrl = route('to-rent.multiple-areas', ['pid' => $multiparam1]);
        }
        ...
        /*if(count($city) < 2){
            ...
            for($r=0; $r < count($city); $r++){
               ...
               // check if city contain number  or  digit
                if(is_numeric($city[$r])){
                  ...
                  //  $customUrl = route('propertydetail',['propertid'=>$propertyDatas->property_id]);
                    break;
                }else{
            ...
            $propertyDatas = '';
             //create  url for multiple city 
            $customUrl = route('to-rent.multiple-areas');
                }
    ...
    public function sideBarCodeForSearch($request)
    {
        ...
        $srNew .= " AND  property_classification = 'residential'";
        //    DB::enableQueryLog();
            ...
            if (!empty($valSearch2)) {
                ...
                for ($ar2 = 0; $ar2 < count($valSearchArray2); $ar2++) {
                    ...
                    if ($propertyDatas !== null) {
                        ...
                        $province = str_replace(" ", "-", $propertyDatas->province);
                        //create  url for single city 
                        $customUrl = route('to-rent', ['url_city' => $suburb, 'url_town' => $town, 'url_province' => $province, 'city' => $valSearchArray2[$ar2]]);
