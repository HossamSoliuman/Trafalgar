@php
    $checkPageType = Request::segment(2);
@endphp
...
@if ($checkPageType != 'multiple-areas')
    ...
    @php
        $suburbTitle = substr(str_replace('-', ' ', Request::segment(2)), 0, 20);
        ...
        if (isset($_GET['city'])) {
            ...
            $cityDesc = str_replace('-', ' ', Request::segment(3));
        }

        $citytown = isset($url_town) ? $url_town : str_replace('-', ' ', Request::segment(3));
    @endphp

    @section('title', 'Commercial Property for sale in ' . $suburbTitle . ', ' . $cityTitle)
    ...
    @section('meta_description',
        ...
        $cityDesc .
        '. The
        largest selection of office, retail & industrial property in ' .
        $suburbTitle)
    @else
    @section('meta_keywords', 'COMMERCIAL MULTIPLE SUBURBS FOR SALE')
    @section('title', 'Commercial property for sale in Multiple Suburbs')
    @section('meta_description',
        'Commercial property for sale in Multiple Suburbs. A selection of office, retail &
    ...
    <section class="guldenland_section">
        <div class="container">
            <div class="heading mb-4">
                ...
                @endphp
                <h2>Property for sale in <?php if (isset($_GET['city']) && $_GET['city'] != '') {
                    echo $text;
                } ?></h2>
            </div>
                ...
                <div class="guldenland_products_listing">
                    ...
                    <div class="guldenland_view" id="showPropertyList" style="display:block">
                                    ...
                                    <div style="cursor: pointer;" onclick="window.location = '{{ $customUrl }}';"
                                        ...
                                        <div class="product_img">
                                            <a title="{{ $propertyData->complex_name . ' ' . $propertyData->unit_number }}"
                                                ...
                                                @if ($propertyData->news_featured_image != '')
                                                    <img class="img-fluid"
                                                        src="{{ $propertyData->news_featured_image }}"
                                        ...
                                        @else
                                            <img class="img-fluid"
                                                ...
                                                alt="{{ $propertyData->headline != '' ? $propertyData->headline : 'propertyDummyImage/dummy_property_image.jpg' }}" />
                                @endif
                        </div>
                        <div class="product_content">
                            ...
                            <small
                                title='{{ ucwords($propertyData->bedrooms . ' ' . $propertyData->property_type . ' ' . $propertyData->mandate_saletype . ' ' . 'in' . ' ' . $propertyData->suburb) }}'>
                                {{ ucwords($propertyData->bedrooms . ' ' . $propertyData->property_type . ' ' . $propertyData->mandate_saletype . ' ' . 'in' . ' ' . $propertyData->suburb) }}
                            </small>
                                ...
                                <div class="product_detail">
                                    ...
                                    @if ($propertyData->bathrooms > 0)
                                        <span title="{{ $propertyData->bathrooms }} Bathroom"><img
                                                ...
                                                alt="icon_bath_new" />{{ $propertyData->bathrooms }} Bathroom</span>
                                    @endif
                                    @if ($propertyData->garages > 0)
                                        <span title="{{ $propertyData->garages }} Garage"><img
                                                src="{{ asset('front/images/icon_parking.png') }}" alt="icon_parking" />
                                            {{ $propertyData->garages }} Garage</span>
                                    @endif
                    ...
                    @if (isset($_GET['city']))
                        ...
                        <h4>
                            @if (!empty($cityArray))
                                ...
                                ?>
                            @endif Commercial property for sale
                        </h4>
                        <ul>
                            @foreach ($sideBarCodeForSearch as $sideBarCodeForSearchs)
                                @if ($cityArray[0] != '')
                                    <li><a
                                            href="{{ route('commercial-property-for-sale', ['url_city' => str_replace(' ', '-', $sideBarCodeForSearchs->suburb), 'url_town' => str_replace(' ', '-', $sideBarCodeForSearchs->town), 'url_province' => str_replace(' ', '-', $sideBarCodeForSearchs->province), 'city' => $cityArray[0], 'ptype' => $sideBarCodeForSearchs->property_type]) }}">
                                            {{ ucfirst($sideBarCodeForSearchs->property_type) }} property for sale in
                                            {{ $sideBarCodeForSearchs->town }} </a></li>
                                @endif
                            @endforeach
                        </ul>
                    @endif
                ...
                <div class="e-magazine-right-content">
                    <div class="each-magazine">
                        ...
                        <div class="magazine-content">
                            ...
                            <p><a href="{{ route('email-property-alert') }}">Sign up</a> for your customised property
                                alerts delivered
                                    ...
                                    @if (!empty($getDistinctSuburb))
                                        @foreach ($getDistinctSuburb as $getDistinctSuburbs)
                                            @php
                                                ...
                                                $customUrl_distinct = route('commercial-property-for-sale', [
...
@push('property-search-commercial-sale-list-js')
    @include('frontPart.js.propertySearchCommercialSaleListJs')
@endpush
