...
class ImportPropertyController extends Controller
{
    ...
    public function importDataFunction($apiKey,$api_city_key_name){
       ...
       // echo count($responseBody);
        //dd($responseBody[0]); // body response
    // $removedProperty    = ['expired', 'inactive', 'rented', 'withdrawn', 'sold - registered'];
     ...
     ///if (!empty($responseBody)) {
      if ($responseBody) {
       ...
       for($r=0; $r < count($responseBody); $r++){
         ...
         //  if(in_array($responseBody[$r]['mandate_status'], $removedProperty) ==  false){
               
        //   }else{
           
            //   $checkPropertyExist =   EntegralApiData::where('property_id',$responseBody[$r]['property_id'])->first();
          ...
          //  }
            ...
            //  $latLngData =  $this->getPropertyLatLong($responseBody[$r]['street_number'],$responseBody[$r]['street_name'],$responseBody[$r]['suburb'],
            //     $responseBody[$r]['town'],
            //     $responseBody[$r]['province'],$responseBody[$r]['country']);
           ...
           $insertEntegralData->news_featured_image = (!empty($photoArrayFeatureImage))? $photoArrayFeatureImage[0]['url_large']:$responseBody[$r]['photo_thumbnail'];
            ...
            //for sending email alert of property 
            // if($checkifPropertyNew == 1){
            //     $this->newPropertyAlertToUser();
            // }
             //for sending email alert of property 
        ...
        //         $checkPropertyImageData =   EntegralPropertyImage::where('property_id',$property_id)->first();
        //     if(!empty($checkPropertyImageData)){
        //          $insertEntegralData =  EntegralPropertyImage::where('property_id',$property_id)->delete();
        //     }
               
        //         for($g=0; $g < count($photoArray); $g++){
                  
        //             $EntegralPropertyImage = new EntegralPropertyImage;
        //             $EntegralPropertyImage->property_id = $property_id;
        //             $EntegralPropertyImage->url_thumb = $photoArray[$g]['url_thumb'];
        //             $EntegralPropertyImage->url_large = $photoArray[$g]['url_large'];
        //             $EntegralPropertyImage->property_photo_description = $photoArray[$g]['description'];
        //             $EntegralPropertyImage->save();
