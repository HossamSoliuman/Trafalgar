...
})(window,document,'script','dataLayer','GTM-PRZJX5P');</script>
...
<noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-PRZJX5P"
...
<script>
    $(function(){
        ...
        window.dataLayer.push({'type': 'Property Management Quote Request','event': 'FormSubmitted' });
         ...
         window.dataLayer.push({'type': 'Report Maintenance Issues','event': 'FormSubmitted' });
       
         @endif
         
           @if(Session::has('formName') && ( Session::get('formName') == 'finance-contact-mail' ))
         window.dataLayer.push({'type': 'Finance','event': 'FormSubmitted' });
       
         @endif
         
          @if(Session::has('formName') && ( Session::get('formName') == 'insure-contact-mail' ))
         window.dataLayer.push({'type': 'Insurance','event': 'FormSubmitted' });
       
         @endif
         
           @if(Session::has('formName') && ( Session::get('formName') == 'quick-contact' ))
            // alert("{{ Session::get('pagename') }}");
         window.dataLayer.push({'type': 'Quick Contact','event': 'FormSubmitted','pagename':'{{  Session::get("pagename") }}' });
       
         @endif
         
          @if(Session::has('formName') && ( Session::get('formName') == 'trafalgar_contactus' ))
        
         window.dataLayer.push({'type': 'Contact Us','event': 'FormSubmitted' });
...
@endif
         ...
         window.dataLayer.push({'type': 'Trafalgar Agent Individual Contact Form','event': 'FormSubmitted','name':'{{ str_replace("-"," ",Session::get("subjectName")) }}' });
         @else
       
          window.dataLayer.push({'type': 'Trafalgar Agent Individual Property Contact Form','event': 'FormSubmitted','property_id':'{{ Session::get("subjectName") }}' });
         @endif
         ...
         window.dataLayer.push({'type': 'Trafalgar Individual Property Application Form','event': 'FormSubmitted','propertyid':'{{ Session::get("propertyId") }}' });
       
         @endif
         
          @if(Session::has('formName') && ( Session::get('formName') == 'Trafalgar Individual Job Application Form' ))
    
         window.dataLayer.push({'type': 'Trafalgar Individual Job Application Form','event': 'FormSubmitted','jobtitle':'{{ Session::get("jobTitle") }}' });
