...
/// code for show and hide price with rent and sale type when page is loaded
    ...
    /// code for show and hide map div
    	...
    	$(".propertySearchButton").click(function () {
            ...
            getSerachForRentForm(city,cityrentdata);
            return false;
              ///new code for tagger search for rent ////
    		}
    	    if(getstype == "for sale"){
    		   getSerachForSaleForm(city,cityrentdata);
            ...
            //  $('.resetFormData')[1].reset();
             $("#cityListForSearchPage").val(null).trigger("change"); 
               //$("#ptype_rent").multiselect( 'refresh' );
                ...
                $('#fminsize_rent').val('');
                $('#fmaxsize_rent').val('');
                $("#ptype_rent").multiselect("clearSelection");
   ...
   function getSerachForRentForm(city,cityrentdata){
       //alert(cityrentdata);
    	     $.ajaxSetup({
                headers: {
    	 ...
    	 function getSerachForSaleForm(city,cityrentdata){
    	     $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
            ...
            $.ajax({
                ...
                data:{city:city,citysaledata:cityrentdata},
                dataType : 'json',
                async : false,
                success:function(data){
 ...
 <script>
        ...
        $(propertyArray.data).each(function(index, element) {
            ...
            if(element.streetNumber!=''){
                validAddress = validAddress+element.streetNumber+" "+element.streetName+","+element.suburb;
            }else{
                validAddress = '';
            }
        	...
        	var singleTownLocationArray = [element.town, element.location_lat, element.location_long,validAddress];
        	...
        	console.log("multipleTownDetailArray="+multipleTownLocationArray);
        
        jQuery(function($) {
            
        	// Asynchronously Load the map API 
        	var script = document.createElement('script');
        	script.src = "https://maps.googleapis.com/maps/api/js?key=AIzaSyDFxIh8LLG3npkRjS4hGeFRthWa0KQq3ts&v=3&callback=initialize";
        ...
        function initialize() {
        	...
        	for(i = 0; i < markers.length; i++) {
        	    ...
        	    console.log("marker3="+markers[i][3]);
        	   /* var dd = searchLatAndLngByStreet(markers[i][3]){
