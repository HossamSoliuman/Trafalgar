...
@if($checkPageType != "multiple-areas")
    ...
    @if (Request::segment(3))
        ...
        @section('meta_description', 'Property to rent in ' . $suburbTitle . ', ' . $cityTitle . ' ' . $property_id . '. The largest selection of flats, farms, townhouses & houses to rent in ' . $suburbTitle . '.')
    @else
        @section('title', 'Property and houses to rent in ' . $locationTitle . ' ' . $property_id)
        @section('meta_description', 'Property to rent in ' . $locationDesc . ' ' . $property_id . '. The largest selection of flats, farms, townhouses & houses to rent in ' . $locationDesc . '.')
    @endif
@else
       ...
       if($url_city == 3 && $url_town == 3 && $url_province == 3){
        ...
        ?>><?php echo $getSearchTaggerData[$key_check]['text']; ?> </option>
                          ...
                          <div class="input-group-append">
                             <button data-formtype="formRentTypeSubmit" data-citydata="cityListForSearchPage" class="btn btn-outline-secondary search-btn propertySearchButton button-addon2" type="button"
                        ...
                        <div class="guldenland_all_product">
                            ...
                            <div style="cursor: pointer;" onclick="window.location = '{{ $customUrl }}';" class="guldenland_each_product">
                                <div class="product_img">
                                   ...
                                   <img title="{{ $propertyData->complex_name." ".$propertyData->unit_number }}" class="img-fluid" src="{{ $propertyData->news_featured_image }}"  alt="{{ ($propertyData->headline !="")?$propertyData->headline:$propertyData->news_featured_image }}" />
                                   @else
                                ...
                                <div class="product_content">
                                   ...
                                   <small title='{{ ($propertyData->bedrooms != 0)?$propertyData->bedrooms." Bedroom ":"Bachelor" }} {{ ucwords($propertyData->property_type." ".$propertyData->mandate_saletype." "."in". " ".$propertyData->suburb) }}' > {{ ($propertyData->bedrooms != 0)?$propertyData->bedrooms." Bedroom ":"Bachelor" }} {{ ucwords($propertyData->property_type." ".$propertyData->mandate_saletype." "."in". " ".$propertyData->suburb) }} </small>
         ...
         @endif
                ...
                <div class="guldenland_sidebar">
                    ...
                    <div class="sidelist">
                        ...
                        <h4> @if(!empty($cityArray)) <?php  //echo $cityArray[0] ?> @endif  Property For Rent  </h4>
                        <ul>
                            @foreach($sideBarCodeForSearch as $sideBarCodeForSearchs)
                            @if($cityArray[0] != "")
                            <li><a href="{{ route('to-rent',['url_city'=> str_replace(' ','-',$sideBarCodeForSearchs->suburb) , 'url_town'=> str_replace(' ','-',$sideBarCodeForSearchs->town) , 'url_province'=> str_replace(' ','-',$sideBarCodeForSearchs->province) ,'city'=> $cityArray[0],'ptype' =>$sideBarCodeForSearchs->property_type  ]) }}"> {{ ucfirst(strtolower($sideBarCodeForSearchs->property_type)) }} to rent in {{$sideBarCodeForSearchs->suburb }}  </a></li>
                            @endif
                            @endforeach
                        </ul>
                        @endif
        ...
        <div class="section-padding">
            <div class="container">
                <div class="row">
                    ...
                    $customUrl_distinct = route('to-rent',['url_city'=>$suburb_distinct,'url_town'=>$town_distinct,'url_province'=>$province_distinct,'city'=>$getDistinctSuburbs->suburb]);
